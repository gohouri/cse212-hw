...
public class LinkedList : IEnumerable<int>
{
    ...
    public void Remove(int value)
    {
        ...
        while (curr is not null)
        {
            if (curr.Data == value)
            {
                // If the node to remove is at the head, use RemoveHead
                if (curr == _head)
                {
                    RemoveHead();
                }
                // If the node to remove is at the tail, use RemoveTail
                else if (curr == _tail)
                {
                    RemoveTail();
                }
                // For any other location, reconnect the links to remove the node
                else
                {
                ...
                return; // We can exit the function after we remove the first match
            }
    ...
    {
        // Search for all nodes that match 'oldValue' by starting at the 
        // head of the list.
        Node? curr = _head;
        while (curr is not null)
        {
            ...
            curr = curr.Next; // Go to the next node to continue searching
        }
    }
